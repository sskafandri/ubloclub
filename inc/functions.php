...
function total_tv_series()
{
    global $conn, $global_settings;

    $sql = "
        SELECT count(`id`) as total_series 
        FROM `tv_series` 
        WHERE `user_id` = '".$_SESSION['account']['id']."'   
    ";
    $query      = $conn->query($sql);
    $results    = $query->fetchAll(PDO::FETCH_ASSOC);
    $data       = $results[0]['total_series'];
...
function sanity_check()
{
    ...
    if($total_licenses == 0){
        ...
        foreach($licenses as $license){
            ...
            // error_log("Total Servers: ".$total_servers);
            // error_log("Total Licenses: ".$total_licenses);

            // lets check servers vs licenses
            if($total_servers > $total_licenses){
                $global_settings['lockdown'] = true;
                $global_settings['lockdown_message'] = '<strong>Too Many Servers</strong> <br><br>You have more servers than licenses. Either remove one or head over to the Support &amp; Billing to order more licenses.';
                return false;
            }
